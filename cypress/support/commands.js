...
Cypress.Commands.add('addCategory', (title, path, parentName) => {
	...
	// Check that category is in updated tree
	cy.wait(2000);
...
/**
 * Toggle sub-categories in the category hierarchy. To toggle a category, all previous categories in the chain has the first expanded.
 * Therefore, the entire category chain(in the correct order) must be passed. E.g to expand "Computer Vision" category, the hierarchy should be:
 * ['Applied Science', 'Computer Science', 'Computer Vision'].
 * @param {Array} hierarchy - An array of category names in the order they should be toggle.
 */
Cypress.Commands.add('toggleSubCategories', (hierarchy) => {
	hierarchy.forEach((categoryName) => {
		cy.contains('tr', categoryName)
			.find('span.sr-only')
			.contains('Expand sub-categories', {includeShadowDom: true})
			.parent('button')
			.click();
	});
});
